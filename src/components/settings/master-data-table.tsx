...
interface MasterDataTableProps {
  ...
  onRefresh: () => void;
  showReorder?: boolean;
  displayMap?: Record<string, string>; // maps value -> display label (for form fields)
}
...
export function MasterDataTable({ title, type, items, onRefresh, showReorder, displayMap }: MasterDataTableProps) {
  ...
  return (
    <div className="space-y-3">
      {/* Add row — like Excel's last empty row */}
              ...
              >
                {showReorder && (
                  <div className="flex gap-0.5 w-8">
                    <button
                      ...
                      onClick={() => index > 0 && swapOrder(item, items[index - 1])}
                      disabled={index === 0}
                    >↑</button>
                    <button
                      className="text-slate-400 hover:text-slate-600 disabled:opacity-20"
                      onClick={() => index < items.length - 1 && swapOrder(item, items[index + 1])}
                      disabled={index === items.length - 1}
                    >↓</button>
                  </div>
                )}

                {/* Value cell — double-click to edit, like Excel */}
                <div className="flex-1 min-w-0">
                  {editingId === item.id ? (
                    ...
                    <span
                      className={cn(
                        "block text-sm py-1.5 px-2 rounded cursor-pointer select-none",
                        "hover:bg-slate-100",
                        !item.isActive && "line-through"
                      )}
                      onDoubleClick={() => { setEditingId(item.id); setEditValue(item.value); }}
