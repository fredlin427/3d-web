...
export async function exportPNG(
  ...
  try {
          ...
          for (const sheet of Array.from(clonedDoc.styleSheets)) {
            try {
              ...
              for (let i = rules.length - 1; i >= 0; i--) {
                if (rules[i].cssText.includes("oklch(") || rules[i].cssText.includes("lab(") || rules[i].cssText.includes("lch(")) {
                  sheet.deleteRule(i);
                }
              }
            } catch (_) {}
    ...
    if (legendItems && legendItems.length > 0) {
      const scale = 3;
      const legendH = Math.ceil(legendItems.length * 16) + 20;
      ...
      for (const item of legendItems) {
        if (item.bold) { ly += 10 * scale; ctx.font = `bold ${11 * scale}px -apple-system, "Segoe UI", Arial, sans-serif`; ctx.fillStyle = "#334155"; }
        else { ctx.font = `${11 * scale}px -apple-system, "Segoe UI", Arial, sans-serif`; ctx.fillStyle = "#64748b"; }
